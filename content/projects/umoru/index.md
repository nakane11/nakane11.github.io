...
従来のハグロボットアームには、ハグにおける汎用性と密着性・安全性との間に2つのトレードオフがありました。
1つ目は、自由度を増やすほど個々のリンクが長くなり、人の体の丸みに沿った滑らかな屈曲が難しくなる「密着性の低下」です。
2つ目は「安全性の低下」です。本研究ではアーム表面に、袋に空気を封入した外装を採用しています。ハグロボットには綿やスポンジが使われることもありますが、空気外装には柔らかさを調整しやすく接触検知にも応用できるという利点があります。しかし従来の汎用アームは骨格内部が部品で埋まっているため、空気を送るチューブを骨格の外に配管する設計が多く見られ、人と密着する際にチューブが損傷するリスクがありました。
そこで本研究では、骨格の短円節化および配管配策の内臓

## ワイヤ駆動による骨格の短円節化

アーム根元に搭載した1つのモータからワイヤで動力を伝達し、5関節を同時に屈曲させる列駆動方式とすることで、各リンクに搭載する部品を削減しました。さらに駆動自由度を、ハグにおいて重要な肩の上下動作と抱きしめるための屈曲動作に限定し、長手方向にねじれる回転をなくすことで内部機構の複雑化を防いでいます。これらの工夫により、長さ80mmのリンクを5つ連ねた5関節での滑らかな屈曲を実現しました。
...
この分散配置を実現するため、給気用電磁弁・気圧センサ・マイコンを実装した基板をまとめた「小型空気圧モジュール」を新たに開発し、各リンクに搭載することで空気袋ごとの内圧を個別制御できるようにしました。さらに、関節を駆動するサーボとこのモジュールの駆動電圧・通信パケット形式を統一することで、全モジュールを3本の線で数珠つなぎに接続でき、配線量も削減しています。
